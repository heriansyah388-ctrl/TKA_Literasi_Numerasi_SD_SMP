        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="flex flex-wrap items-center gap-2 text-xs">
            ...
            <select
              ...
              className="px-3 py-1.5 rounded-lg border border-slate-200 bg-slate-50 text-slate-700 font-medium text-xs focus:outline-hidden focus:ring-1 focus:ring-indigo-500"
            >
          ...
          <div className="relative w-full lg:w-72">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-1.5 text-xs rounded-xl border border-slate-200 bg-slate-50 focus:bg-white focus:outline-hidden focus:ring-1 focus:ring-indigo-500"
            />
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {filteredItems.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center">
            ...
            <h3 className="text-sm font-semibold text-slate-700">Tidak ada butir soal yang sesuai filter</h3>
            <p className="text-xs text-slate-500 mt-1">Coba ubah opsi jenjang, domain, atau kata kunci pencarian Anda.</p>
          </div>
        ) : (
          filteredItems.map((soal, idx) => {
            ...
            return (
              <div
                key={soal.id}
                className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden transition-all duration-200 hover:border-indigo-200"
              >
                {/* Item Header */}
                <div className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 bg-slate-50/50">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-xs text-slate-600 font-medium">
                      Subdomain: <strong className="text-slate-800">{soal.subdomain}</strong>
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 self-end sm:self-auto">
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-2.5 py-1 text-xs text-slate-600 hover:text-slate-900 bg-white border border-slate-200 rounded-lg hover:bg-slate-50 transition-colors cursor-pointer"
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1 px-3 py-1 text-xs font-medium text-indigo-700 bg-indigo-50 border border-indigo-200 rounded-lg hover:bg-indigo-100 transition-colors cursor-pointer"
                    >
                ...
                <div className="p-5 sm:p-6 space-y-4">
                  {/* Stimulus */}
                  <div className="bg-amber-50/40 rounded-xl p-4 border border-amber-200/50 text-xs sm:text-sm text-slate-800 leading-relaxed font-serif">
                    <div className="text-[11px] font-sans font-bold text-amber-900 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                        ...
                        {Object.entries(soal.opsi).map(([optKey, optText]) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              {isCorrect && (
                                <span className="ml-auto text-[11px] font-bold text-emerald-700 shrink-0">
                                  Kunci Jawaban
                                </span>
                              )}
                            </div>
                  ...
                  {isExpanded && (
                    <div className="mt-4 pt-4 border-t border-slate-200 space-y-3 bg-slate-50/80 -mx-5 -mb-5 sm:-mx-6 sm:-mb-6 p-5 sm:p-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                        <div className="space-y-2">
                          <div className="bg-white p-3 rounded-xl border border-slate-200">
                            ...
                            <p className="text-slate-600">{soal.kompetensi}</p>
                          </div>
                          ...
                          <div className="bg-white p-3 rounded-xl border border-slate-200">
                            ...
                            <p className="text-slate-600">{soal.indikator}</p>
                          </div>
                        </div>
                        ...
                        <div className="space-y-2">
                          <div className="bg-white p-3 rounded-xl border border-slate-200">
                            ...
                            <p className="text-amber-800">{soal.kesalahan_umum || 'Siswa terkecoh distraktor berbasis informasi tersirat tanpa verifikasi.'}</p>
                          </div>
                          ...
                          <div className="bg-white p-3 rounded-xl border border-slate-200">
                            ...
                            <p className="text-slate-600">
                              Konteks: <strong>{soal.konteks}</strong> • Level: <strong>{soal.level_kognitif}</strong>
                            </p>
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="bg-white rounded-xl border border-slate-200 p-3.5 text-xs">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                            {Object.entries(soal.alasan_distraktor).map(([dKey, dDesc]) => (
                              <div key={dKey} className="p-2 rounded-lg bg-slate-50 border border-slate-200">
                                <span className="font-bold text-slate-700 mr-1.5">Opsi {dKey}:</span>
                                <span className="text-slate-600">{dDesc}</span>
                              </div>
                            ))}
                          </div>
                        </div>
